...
export default function ActiveAlertsPanel() {
  const { alerts, truncated, resolveIncident } = useAlerts();
  ...
  const activeIncidents = groupByIncident(alerts.filter(a => a.status === "active")).slice(0, 3);
  ...
  return (
    <div style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24, boxShadow: "0 1px 2px 0 rgba(0, 0, 0, 0.03)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111827" }}>Active Incidents</span>
        <Link
          href={alertsHref}
          onClick={(e) => {
            if (isNavigating) {
          ...
          return (
            <div key={incident.id} style={{ backgroundColor: config.bg, border: `1px solid ${config.border}`, borderRadius: 8, padding: "16px", display: "flex", alignItems: "flex-start", gap: 12, opacity: isResolving || isNavigating ? 0.6 : 1, transition: "opacity 0.2s", pointerEvents: isNavigating ? "none" : "auto" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
                <span style={{ fontSize: 12, fontWeight: 500, color: "#6b7280" }}>{incident.alerts[0].time}</span>
                ...
                <button
                  ...
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "#ffffff"; e.currentTarget.style.borderColor = "#d1fae5"; }}
                >
      ...
      {truncated && (
        <div style={{ marginTop: 12, fontSize: 12, color: "#92400e" }}>
          Not every active alert could be loaded, so endpoint counts here may be short.
        </div>
      )}
    </div>
  );
